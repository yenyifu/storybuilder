...
function makeInitialSpreads(storyText: string): Spread[] {
  ...
  if (!spreads.length) {
    spreads.push({ id: crypto.randomUUID(), left: pageFromText(""), right: pageFromText("") })
  }
  return spreads
}
...
export default function LayoutBuilderPage() {
  ...
  function updateBlock(side: "left" | "right", blockId: string, blockPartial: Partial<PageBlock>) {
    setSpreads((prev) =>
      prev.map((sp, i) => {
        if (i !== currentIndex) return sp
        const page = sp[side]
        const nextBlocks = (page.blocks || []).map((b) => (b.id === blockId ? { ...b, ...blockPartial } : b))
  ...
  function addSpread(afterIndex?: number) {
    ...
    setCurrentIndex((i) => (afterIndex != null ? afterIndex + 1 : spreads.length))
  }
      ...
      <div className="flex h-full">
        ...
        <div className="relative flex-1 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3">
            <h1 className="text-lg font-semibold text-gray-900">Layout Builder</h1>
